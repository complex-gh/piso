...
func TestWriteWorkerComposeRendersPerWorkerEnv(t *testing.T) {
	...
	for _, name := range []string{"Dockerfile", "entrypoint.sh", "planning-watch.sh", "loopback-forward.py", "context-watch.sh", "ports-watch.sh", "activity-watch.sh", "informant-help.sh", "INFORMANT.md", "CAPABILITIES.md", "MONITOR.md", "monitor-loop.sh", "package.json"} {
